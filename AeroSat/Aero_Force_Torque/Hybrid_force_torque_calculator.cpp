 ...
 * @param aero
 */
int HybridForceTorqueCalculator::calc_aero_torque_force(const Eigen::Vector3f& v_rel__m_per_s, AeroConditions aero, Eigen::Vector3f& torque__Nm, Eigen::Vector3f& force__N) {
	std::vector<float> triangle_visibility(m_satellite->get_num_triangles(), 0.0f);
	torque__Nm = Eigen::Vector3f::Zero();
	force__N = Eigen::Vector3f::Zero();
	...
	for (unsigned int i = 0; i < m_satellite->get_num_triangles(); i++){
		...
		float area = areas[i];
		float visibility = triangle_visibility[i];
		Eigen::Vector3f aero_force__N;
		Eigen::Vector3f aero_torque__Nm;
		m_gsi_model->calc_aero_force_and_torque(area, normal, centroid, v_rel__m_per_s.normalized(), 0.0f, aero, aero_force__N, aero_torque__Nm);
		for (int j = 0; j < 3; j++) {
