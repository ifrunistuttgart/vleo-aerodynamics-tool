...
Prerequisites
- CMake (>= 3.15, newer recommended)
- A C++20-capable compiler (MSVC on Windows is tested)
- vcpkg (manifest mode is used to manage dependencies)
- (Optional) MATLAB (for MATLAB bindings; required only when building the MATLAB targets)

Quick build (Windows, using the provided CMake presets)

Open PowerShell in the project root and run:

```powershell
# configure (example: 64-bit debug)
cmake --preset x64-debug
...
Notes:
- The project defines configure presets in `CMakePresets.json` (e.g. `x64-debug`, `x64-release`).
- The presets set the vcpkg toolchain file and manifest mode; ensure `VCPKG_ROOT` is set to your vcpkg installation directory before configuring.
- To enable MATLAB bindings, configure with `-DBUILD_MATLAB_BINDINGS=ON` (see the MATLAB section below).
